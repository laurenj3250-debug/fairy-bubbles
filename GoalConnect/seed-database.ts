 ...
 * Run with: npx tsx seed-database.ts
 */
...
async function seedDatabase() {
  ...
  try {
    ...
    if (existingUsers.length > 0) {
      ...
      console.log(`Found user: ${existingUsers[0].name}`);
      process.exit(0);
    }
    ...
    // Monthly Goals
    console.log('🎯 Creating monthly goals...');
    const monthlyGoals = [
      {
        userId: user.id,
    ...
    // Weekly Habits
    console.log('📅 Creating weekly habits...');
    const weeklyHabits = [
      {
        userId: user.id,
    ...
    console.log(`\n✅ Go to your app: https://fairy-bubbles.vercel.app`);
